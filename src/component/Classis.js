...
const Classic = () => {
  ...
  useEffect(() => {
    const fetchClassicBooks = async () => {
      try {
        ...
        setClassicBooks(filteredBooks);
      } catch (err) {
        setError("Failed to fetch classic books.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredBooks = classicBooks.filter((book) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">Classic Books: Now</h1>
